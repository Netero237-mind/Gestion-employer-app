# Gestion-employer-app
Application capable de créer, modifier , supprimer des employer d'une plateforme.
...
Python Django Vent arrière CSS DaisyUI SQLite Statut
...
📱 Conception réactive	Interface adaptée ordinateurs, tablettes et mobiles
🎨 Thème Aqua	UI professionnelle et reposante via DaisyUI
🔒 Validation des données	Contrôle d’intégrité à la saisie via DjangoModelForm
🛠️ Technique de la pile
SIGE
...
Frontend	CSS Vent Arrière + DaisyUI	Stylisage responsive & components UI
Architecture	MVT	Séparation des responsabilités
Base de données	SQLite	Persistance des données (dev)
Modèles	Modèles Jinja2 / Django	Rendu dynamique des vues
🧱 Architecture MVT
┌──────────────────────────────────────────────────┐
│                   Navigateur                     │
...
Toutes les opérations passent par Django , qui assure la validation côté serveur et prévient les erreurs de saisie et les injections.ModelForm

Héritage de templates
Un unique fichier centralise la charte graphique. Toutes les vues héritent de ce template, garantissant cohérence et non-répétition du code HTML.base.html
